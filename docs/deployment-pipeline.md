...
已提供代码和脚本。尚未配置真实服务器，不能把脚本校验视为云端部署验收。默认 Linux x86_64、Docker Engine + Compose v2、PostgreSQL 18、持久化磁盘图片存储（可切换 S3）。构建在 GitHub Runner 完成，轻量服务器只拉镜像并运行。
...
- PostgreSQL 使用持久卷，图片使用 image-data 持久卷（或所配置的 S3 桶）；不会删除数据卷或覆盖 .env 中的密码。
- 磁盘模式同时备份图片压缩包；S3 模式需单独配置桶备份。备份位于 /opt/goods-island/backups，需要自行设置异地备份和保留周期。
